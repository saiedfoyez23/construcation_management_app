...
class GetPostPourInspectionReportController extends GetxController {
  ...
  String projectId;
  GetPostPourInspectionReportController({required this.projectId});
  ...
  Future<Uint8List> generatePdf(GetPostPourInspectionReportResponse data) async {
    isPdf.value = true;
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        build: (pw.Context context) => [
          ...
          pw.Text("Project: ${getProjectDetailsResponseModel.value.data?.name ?? ''}",
  ...
  /// ---------- SAVE + OPEN ----------
  Future<void> createAndOpenPdf(GetPostPourInspectionReportResponse response) async {
...
// Generate Excel file
  Future<void> generateAndOpenExcel(GetPostPourInspectionReportResponse data) async {
    ...
    sheet.getRangeByName('B3').setText('${getProjectDetailsResponseModel.value.data?.name ?? ''}');
