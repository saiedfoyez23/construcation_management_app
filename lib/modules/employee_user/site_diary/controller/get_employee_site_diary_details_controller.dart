...
class GetEmployeeSiteDiaryDetailsController extends GetxController {
  RxBool isLoading = false.obs;
  RxBool isPdf = false.obs;
  RxBool isExcelOpen = false.obs;
  RxList<EmployeeSiteDiaryDetailsWorkforce> workforce = <EmployeeSiteDiaryDetailsWorkforce>[].obs;
  RxList<EmployeeSiteDiaryDetailsEquipment> equipment = <EmployeeSiteDiaryDetailsEquipment>[].obs;
  RxList<EmployeeSiteDiaryDetailsTask> taskList = <EmployeeSiteDiaryDetailsTask>[].obs;
  ...
  String siteDiaryId;
  String projectId;
  GetEmployeeSiteDiaryDetailsController({required this.siteDiaryId,required this.projectId});
  ...
  Future<void> getEmployeePdfExcelController({required String siteDiaryId,required bool isExcel,required BuildContext context}) async {
    try {
      if(isExcel == true) {
...
class EmployeeSiteDiaryDetailsTask {
  final String name;
  final List<EmployeeSiteDiaryDetailsWorkforce> workforce;
  final List<EmployeeSiteDiaryDetailsEquipment> equipment;

  EmployeeSiteDiaryDetailsTask(this.name, this.workforce, this.equipment);

}

class EmployeeSiteDiaryDetailsWorkforce {
  final String typeId;
  final int quantity;
  final int duration;

  EmployeeSiteDiaryDetailsWorkforce(this.typeId, this.quantity, this.duration);
}

class EmployeeSiteDiaryDetailsEquipment {
  final String typeId;
  final int quantity;
  final int duration;

  EmployeeSiteDiaryDetailsEquipment(this.typeId, this.quantity, this.duration);
}


extension SiteDiaryMapper on GetEmployeeSingleSiteDiaryDetailsResponse {
  List<EmployeeSiteDiaryDetailsTask> toTaskList() {
    if (tasks == null) return [];

    return tasks!.map((task) {
      final workforceList = task.workforces?.map((wf) {
        return EmployeeSiteDiaryDetailsWorkforce(
          wf.workforce?.sId ?? "",
          (wf.quantity is int)
              ? wf.quantity
      ...
      final equipmentList = task.equipments?.map((eq) {
        return EmployeeSiteDiaryDetailsEquipment(
          eq.equipment?.sId ?? "",
          (eq.quantity is int)
              ? eq.quantity
      ...
      return EmployeeSiteDiaryDetailsTask(
        task.name ?? "",
        workforceList,
        equipmentList,
      );
