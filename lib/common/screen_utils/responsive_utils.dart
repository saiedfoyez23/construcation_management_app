...
class ResponsiveUtils {
  ...
  static double _getScaledDimension({
    ...
    final designSize = isHeight
        ? DesignUtils.designHeight(context: context)
        : DesignUtils.designWidth(context: context);
...
class DesignUtils {
  static num designHeight({required BuildContext context}) {
    return MediaQuery.of(context).orientation == Orientation.portrait ? 812 : 375;
  }

  static num designWidth({required BuildContext context}) {
    return MediaQuery.of(context).orientation == Orientation.portrait ? 375 : 812;
  }
}
